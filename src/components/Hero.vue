<template>
  <section
    id="hero"
    class="min-h-screen flex items-center justify-center py-12 sm:py-20 md:py-32"
    aria-label="Hero"
  >
    <div
      v-reveal.once="{ threshold: 0.15 }"
      class="relative w-full max-w-3xl text-center px-4 sm:px-6 md:px-8 py-10 sm:py-12
    ...
    >
      <svg
        class="pointer-events-none absolute -top-16 sm:-top-20 left-1/2 -translate-x-1/2
               w-64 h-64 sm:w-96 sm:h-96 md:w-[480px] md:h-[480px]
               opacity-20 dark:opacity-10 -z-10"
        viewBox="0 0 600 600"
        fill="none"
      ...
      <div class="flex justify-center -mt-16 sm:-mt-24">
        ...
        >
          <div
            class="absolute -inset-[1px] sm:-inset-[2px] md:-inset-[6px] rounded-full p-[1px]
                   bg-gradient-to-tr from-yellow-400 via-pink-400 to-violet-500"
            aria-hidden="true"
          ></div>
      ...
      <p class="mt-3 sm:mt-4 max-w-prose mx-auto text-xs sm:text-sm md:text-base text-gray-600 dark:text-gray-200/90 italic">
        {{ quote }}
      </p>
      ...
      <div class="mt-6 sm:mt-8 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
        ...
        <button
          ...
          class="w-full sm:w-auto px-4 py-2 rounded-md text-sm
...
@media (prefers-reduced-motion: reduce) {
  ...
  .hover\:-translate-y-0\.5, .hover\:scale-\[1\.02\], .transition-all, .transition-transform { transition: none !important; }
}
